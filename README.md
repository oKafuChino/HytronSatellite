...
添加成功后，Bot 会记录当前状态作为初始基线。之后只要该节点的库存配置发生变化，Bot 就会主动通知你，并列出变化前后的资源值和差值。监控字段包括 CPU、内存、磁盘、IP 和 VM 名额；有货状态变化也会单独列出。
